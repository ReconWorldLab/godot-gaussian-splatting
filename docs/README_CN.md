...
本仓库本身就是一个 Godot 工程：克隆后用 Godot `4.3+` 直接打开仓库根目录，等待首次导入完成，按 F5 即可。`samples/demo.tscn` 是主场景：一个交互式的重新打光沙盒——一盏灯绕着示例资源转，地面接住它的投影，旁边放一个普通 Godot 立方体，方便对比同一盏灯打在 splat 和真实几何体上的效果。所有选项都是一个按键，画面上有按键说明。

演示会在首次运行时自行烘焙光照代理（几秒）并缓存，之后秒开。`samples/demo.gd` 用代码搭建了整个场景，同时也是 API 的完整示例。
...
交互试用：内置演示（`samples/demo.tscn`，本工程的主场景）会让一盏灯绕着示例资源转，所有选项都能实时切换。
...
- 支持导入 `.ply`、`.compressed.ply`、`.splat`、`.sog`、`.glb` 和 `.usdz` 格式的 Gaussian 资源。
...
- 支持 NuRec 风格的 USDZ 归档（Scaniverse / NVIDIA Omniverse 导出）：ZIP 内包含 USD 场景描述和一个 gzip 压缩的 MessagePack `.nurec` 载荷，Gaussian 张量以 float16 存储。
...
- `samples`：演示场景（`demo.tscn` + `demo.gd`）、示例 Gaussian 资源和媒体文件。
- `tests`：CI 使用的 headless 测试（冒烟、碰撞管线、Raster 排序器/数据纹理、后端选择器、光照烘焙与光源 rig），以及视觉 A/B 工具和它的极简参考场景。
